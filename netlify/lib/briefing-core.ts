...
/**
 * SAM PHASE 1.1 — MORNING BRIEFING SHARED LOGIC
 *
 * All the data-fetching, Claude synthesis, email delivery, and blob
 * archiving lives here. Two callers:
 *   - briefing-daily.mts (scheduled cron at 6 AM ET)
 *   - briefing.mts       (HTTP endpoints for manual trigger + history)
 *
 * Netlify doesn't let a single function be both scheduled AND path-routed,
 * which is why we split. Logic itself is identical.
 */

export const OWNER_NAME = "Dr. Hatami";
export const OWNER_EMAIL = "admin@edupolicy.ai";
export const BRIEFING_FROM = "briefing@edupolicy.ai";
...
export async function fetchM365Inbox24h(): Promise<string> {
  try {
...
export async function fetchGmail24h(): Promise<string> {
  try {
...
export async function fetchM365CalendarToday(): Promise<string> {
  try {
    const token = await getM365Token();
...
export async function fetchOpenTasks(): Promise<string> {
  try {
    ...
    const prioritized = tasks
      ...
      .sort((a, b) => {
        ...
        const pOrder: any = { urgent: 0, high: 1, medium: 2, low: 3 };
...
export async function fetchRecentZoomTranscripts(): Promise<string> {
  try {
...
export async function synthesizeBriefing(sections: {
  m365Inbox: string;
  gmail: string;
  calendar: string;
  tasks: string;
...
export async function sendBriefingEmail(markdownBody: string, dateLabel: string) {
...
export async function buildAndSendBriefing(): Promise<{
  ok: true;
  key: string;
  preview: string;
  durationMs: number;
  ...
  const dateKey = startedAt.toLocaleDateString("en-CA", { timeZone: TZ });
  ...
  return { ok: true, key: dateKey, preview: briefing.slice(0, 300), durationMs };
}
